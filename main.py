...
def draw_init_screen():
    ...
    while waiting:
        ...
        draw_text("Welcome to Arrow Game!" , "Courier" , 40 , WIDTH/2 , HEIGHT/3 , WHITE)
        draw_text("Press ↑ ↓ ← → to eliminate arrows" , "Courier" , 35 , WIDTH/2 , HEIGHT/2 , WHITE)
        draw_text("Press Enter to start" , "Courier" , 30 , WIDTH/2 , HEIGHT*2/3 , WHITE)
        draw_text("(Press Space to pause)" , "Courier" , 25 , WIDTH/2 , HEIGHT*7/9 , WHITE)
        pygame.display.update()
...
def draw_failure(SCORE , VELOCITY , ARROWS_LINE_NUMBER , all_sprites):
    ...
    while waiting:
        ...
        # Draw record #
        draw_text(f"You eliminated {SCORE} arrows !" , "Courier" , 45 , WIDTH/2 , HEIGHT/2 , BLUE , 1)
        draw_text("Press Enter to play again" , "Courier" , 35 , WIDTH/2 , HEIGHT*3/4 , BLUE , 1)
        if SOCLOSE:
            draw_text("So close to break the record :(" , "Courier" , 40 , WIDTH/2 , HEIGHT/4 , RED , 1)
        if NEWRECORD:
            draw_text("New Record !" , "Courier" , 50 , WIDTH/2 , HEIGHT/4 , RED , 1)
        pygame.display.update()
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                # pygame.quit()
...
def pause(SCORE , VELOCITY , ARROWS_LINE_NUMBER , all_sprites):
    ...
    while waiting:
        ...
        draw_text("Pause" , "Courier" , 100 , WIDTH/2 , HEIGHT/2 , RED , 3)
        draw_text("F11 : Full Screen" , "comicsans" , 20 , WIDTH - 100 , 80 , BLACK)
        draw_text("Esc : End the game" , "comicsans" , 20 , WIDTH - 100 , 120 , BLACK)
        if sound_check % 2:
